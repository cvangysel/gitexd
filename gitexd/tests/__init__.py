...
class GitTestHelper(unittest.TestCase):
    ...
    def pushRepository(self, repository, password = None, keyFile = None):
        ...
        reactor.spawnProcess(p, repository.git, [repository.git, "push", "--all", "origin"], path=repository.path, usePTY=True, env=self._getEnvironmentVars(keyFile))
    ...
    def pullRepository(self, repository, password = None, keyFile = None):
        ...
        p = GitProcess(d, password)
        reactor.spawnProcess(p, self.repository.git, [self.repository.git, "pull", "origin", "master:master", "second-branch:second-branch"], path=self.repository.path, usePTY=True, env=self._getEnvironmentVars(keyFile))
